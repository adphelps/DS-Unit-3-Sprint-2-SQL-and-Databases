...
- In the Northwind database, what is the type of relationship between the
  ...
  ~There is no relationship between the two. They would need to share a common 
  column to have a relationship.
  
  
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  ~document stores are very flexible and don't have a forced schema. This makes 
  it highly appropriate for things like product catalogues, and the relatively 
  good performance and availability makes it good for real-time data collection.
  
  Document stores may not be such a good idea when you have many related tables,
  because there are no joins with a document store. The best you can get is sub-
  documenting, but the sub-document can only be related to the parent object it's
  been embedded into. It also isn't very ACID compliant, which may or may not be
  an issue depending on your use case.
  
- What is "NewSQL", and what is it trying to achieve?
  ~NewSQL is basically a relatively new way to manage a database. The main goal is
  to provide the scalability of NoSQL systems while maintaining ACID compliance.
  It offers modern capabilites and modern architecture using SQL to access and 
  modify data.
